...
WHERE EMP_STATUS = 1
AND YSN_SALARYHOLD = 0
AND (DEPARTMENT_ID IS NULL OR DESIGNATION_ID IS NULL OR LOCATION_ID IS  NULL OR JOB_TYPE_ID IS  NULL OR JOB_TYPE IS NULL)

--IS_TOP_MANAGEMENT CAN'T BE NULL 0 FOR BELLOW MANAGER 1 FOR ABOVE MANAGER
SELECT * 
FROM EMP_DESIGNATION
WHERE IS_TOP_MANAGEMENT IS NULL;
...
--PL list check
SELECT EMPLOYEE_ID, 7 MONTH, 2024 YEAR, B.*
FROM LEAVE_APPLICATION A, EMP_PROFILE_ALL B
WHERE A.EMPLOYEE_ID = B.EMP_ID
AND LEAVE_TYPE_ID = 7
AND APPROVED_STATUS = 'Y'
AND TO_NUMBER(TO_CHAR(APPLIED_FROM_DATE,'MM')) = 7 +1
AND TO_NUMBER(TO_CHAR(APPLIED_FROM_DATE,'RRRR')) = 2024;

--Dublicate allowance check 
SELECT EMPLOYEE_ID, ALLOWANCE_TYPE , NVL(COUNT(EMPLOYEE_ID),0)
FROM ALLOWANCE
WHERE STATUS = 1
GROUP BY EMPLOYEE_ID, ALLOWANCE_TYPE
HAVING COUNT(*) > 1;
...
----------After generate salary  count employee 
SELECT A.EMP_ID, A.COMP_ID, A.LOCATION_ID, A.PAYMENT_TYPE, B.GROSS_SALARY
FROM EMP_INFO A, GRADE_EMP_SALARY B
WHERE A.EMP_ID = B.EMP_ID
AND     A.EMP_STATUS = 1
AND     A.YSN_SALARYHOLD = 0
AND     A.PAYMENT_TYPE IS NOT NULL
AND     A.PAYMENT_TYPE NOT IN ('P')
AND     A.COMP_ID NOT IN (105,13)
AND     A.LOCATION_ID NOT IN (131,96,97,423,426,432,439,433,462,479,480,481,482)
AND     A.JOINING_DATE < '31-JUL-2024'
AND     B.TILL_DATE IS NULL


SELECT * 
FROM TEMP_MONTHLY_SALARY_GENERATE

---IF NOT MATCH 
SELECT * 
FROM TEMP_ATTENDANCE_PROCESS
WHERE ELIGIBLE_FOR_SALARY  = 0